...
Clear[NaiveDrawFunction]
NaiveDrawFunction[accoc_]:=Module[{
layersWithDummyVertices = accoc["layers with dummies"],
arcsWithDummyVertices = accoc["graph with dummies"],
start = accoc["first dummy"],
vertices=GroupBy[Flatten@accoc["layers with dummies"],#<accoc["first dummy"]&],
coords,
result
},{
coords=getNaiveCoords[layersWithDummyVertices];
result = Graph[
Flatten[layersWithDummyVertices],
arcsWithDummyVertices,
VertexShapeFunction-> {"Triangle",x_/;x>=start->"Circle"},
VertexSize->{"Scaled",1/Length[Flatten@layersWithDummyVertices]},
EdgeStyle->Directive[Black,Thin],
VertexCoordinates->Flatten[coords,1],
VertexStyle->{Red,x_/;x>=start->White},
...
(
	Module[
		...
		Which[approach==="Naive",
		 result=NaiveDrawFunction[GetCoordsResult],
		 approach==="WM", 
