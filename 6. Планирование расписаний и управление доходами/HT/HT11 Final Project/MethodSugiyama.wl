...
If[Element[\[Delta]h,PositiveReals]==False,
Message[GetCoordinates::option, \[Delta]h]; 
Abort[]];
If[Element[\[Delta]w,PositiveReals]==False,
Message[GetCoordinates::option, \[Delta]w]; 
Abort[]];
If[MatchQ[weights,{_?IntegerQ,_?IntegerQ,_?IntegerQ}]==False, Message[GetCoordinates::option2, weights];Abort[]];
...
(
	Module[
		...
		Which[approach==="WM", 
		result=Graph[Graph[GetCoordsResult["graph with dummies"],VertexLabels->"Name",ImageSize->Medium, VertexCoordinates->GetCoordsResult["coords"]]]
		];
		result
